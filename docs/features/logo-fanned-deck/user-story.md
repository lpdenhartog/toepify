...
### A5. PWA app-icon — switched from Option 3 to Option 1 (post-delivery, 2026-06-11)

After delivery, the stakeholder noted the PWA home-screen icon (Option 3
`LogoTile`, the inverted green tile) was a *different mark* from the header /
favicon (Option 1 fanned deck). Option 3 was a pipeline-introduced choice (a
conventional "filled app-tile") that the handoff README explicitly excluded
("ignore" Options 2/3 — Option 1 is the canonical brand). Decision: **the app
icon now uses the Option 1 mark** for one consistent brand across header,
favicon, and home screen.

- The committed PNGs (`client/public/icons/icon-{192,512,maskable-512}.png`) are
  regenerated by `scripts/generate-icons.mjs` as the Option 1 fanned deck on a
  **casino-green tile** (`#206848`) with cream cards (`#f0eadd`) — the cards pop
  "on the felt". On the green tile the green card stroke only shows as the
  separator between overlapping cards, so the stroke is thinned to **5** (vs the
  favicon/header mark's 8) for a tighter, cleaner fan. The header and favicon
  stay the cream/light green-outline mark; only the home-screen icon is the green
  tile. (Earlier iterations used a cream `#ece5d6` tile; the stakeholder chose the
  green tile after a side-by-side comparison, for stronger home-screen presence.)
- The mark is scaled to ~1.2 (any) / 0.84 (maskable) so the card edges sit near
  the tile edge with a small margin (the Option 1 viewBox carries ~30px built-in
  padding, hence scaling past 1.0).
- The maskable variant is a **full-bleed square** (no rounded corners → no
  transparent corners; the green fills edge-to-edge) with the mark inside the
  ~80% safe zone.
- `LogoTile.tsx` (Option 3) is now unused and was **deleted**. This also resolves
  the two non-blocking code-review follow-ups (geometry duplication between
