...
- The committed PNGs (`client/public/icons/icon-{192,512,maskable-512}.png`) are
  ...
  filled rounded tile that reproduces the in-app header context: tile background
  `#ece5d6` (app page bg `--bg`), paper cards `#f0eadd`, green `#206848` linework.
- The maskable variant is now a **full-bleed square** (no rounded corners → no
  transparent corners) with the mark inside the ~80% safe zone, correcting the
  earlier rounded-tile maskable.
