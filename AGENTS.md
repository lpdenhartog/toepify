...
- **When implementing a new feature or fix, check whether additional tests (unit or E2E) should be added or updated. Ask the user if those tests should be built when the scope is unclear.**
- Keep this file aligned with `CLAUDE.md` when repository-wide guidance changes.
...
- `npm run dev` — run the server and client together in development mode
- `npm run build` — build the client and server workspaces
- `npm test` — run server and client unit tests
- `npm run test:e2e` — run Playwright E2E tests
- `npm run db:init` — initialize the database schema with `server/src/db/schema.sql`
